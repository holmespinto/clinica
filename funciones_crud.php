...
return $nombre;								
 ...
 function NombresMedicamento($drug_id,$conexion){
	if ($conexion->connect_error) {
		die("Ha fallado la conexión: " . $conexion->connect_error);
	}									
		$sql="SELECT * FROM farmacia_medicamentos WHERE drug_id='".$drug_id."'";	
		$rows = $conexion->query($sql);
	if ($rows->num_rows > 0) {
		// Salida de la consulta mediante el ciclo WHILE
		while($mat = $rows->fetch_assoc()) {
...
function TotalMedicamentoVendidoMes($id_drog,$drog){
 
	$stotal=0;
	foreach ($drog as &$value) {
			if($id_drog==$value){
				$stotal++;
			}
	}		
...
function cargeIdsMedicmentoMes($finicial,$ffinal,$conexion){
	$array=array();
	$idss='';
	if ($conexion->connect_error) {
		die("Ha fallado la conexión: " . $conexion->connect_error);
	...
	if ($rows->num_rows > 0) {
		...
		while($row = $rows->fetch_assoc()) {
			$array[]=$row['id_medicamentos'];
			
		}
	}
	$idss=implode(",", $array);

	return array_filter(explode(",", $idss));
}
...
return array($id,$precio_real,$precio_com,$precio_unidad,$status);								
 ...
 function DatosEmpresa($conexion){
	$datos=array();
...
if ($rows->num_rows > 0) {
	while($mat = $rows->fetch_assoc()) {
		$datos['name']=$mat['name'];
		$datos['phone']=$mat['phone'];
		$datos['fax']=$mat['fax'];
		$datos['street']=$mat['street'];
		$datos['state']=$mat['state'];
		$datos['country_code']=$mat['country_code'];
		$datos['city']=$mat['city'];

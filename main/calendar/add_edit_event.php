...
if (isset($_GET['liq'])) {
    $_GET['liq'] = $_GET['liq'] == "true" ? true : false;
}
...
} elseif (!empty($_GET['liq'])) {
    //providers tab
    $collectthis = collectValidationPageRules("/interface/main/calendar/add_edit_event.php?liq=true");
} else { //patient tab
...
if (!empty($_GET['liq']) && ($_GET['liq'] == true)) {
    $cattype = 2;
}
if ($_GET['group'] == true) {
    $cattype = 3;
...
//
$var_index = 0;
function echoLine($iname, $date, $charges, $ptpaid, $inspaid, $duept, $encounter = 0, $copay = 0, $patcopay = 0, $tarifa_id = 0, $ures,$defaultProvider)
{
    global $var_index;
    $var_index++;
    $balance = bucks($charges - $ptpaid - $inspaid);
    $balance = (round($duept, 2) != 0) ? 0 : $balance;//if balance is due from patient, then insurance balance is displayed as zero" . text(bucks($charges)) . "
    $encounter = $encounter ? $encounter : '';
	if($tarifa_id==1){$carga='Por definir';}else{$carga=bucks($charges);}
    echo " <tr id='tr_" . attr($var_index) . "' >\n";
   echo " <form method='post' name='theform' id='theform' onsubmit='return SubmitForm()'>\n";
   echo "  <td>" . text(oeFormatShortDate($date)) . "</td>\n";
    echo "  <td class='text-center' id='" . attr($date) . "'>" . text($encounter) . "</td>\n";
    echo "  <td class='text-center' id='td_charges_$var_index' ><input type='text' class='form-control' name='charges_" . attr($iname) . "'  id='charges_" . text($iname) . "' " .
        " value='" . text($carga) . "' onchange='coloring();calctotal()'  autocomplete='off' " .
        "onkeyup='calctotal()'/></td>\n";
    echo "  <td class='text-center' id='td_inspaid_$var_index' ><input type='text' class='form-control' name='inspaid_" . attr($iname) . "'  id='inspaid_" . text($iname) . "' " .
        " value='" . text(bucks($inspaid * -1)) . "' onchange='coloring();calctotal()'  autocomplete='off' " .
        "onkeyup='calctotal()'/></td>\n";
    echo "  <td class='text-center' id='td_ptpaid_$var_index' ><input type='text' class='form-control' name='ptpaid_" . attr($iname) . "'  id='ptpaid_" . text($iname) . "' " .
        " value='" . text(bucks($ptpaid * -1)) . "' onchange='coloring();calctotal()'  autocomplete='off' " .
        "onkeyup='calctotal()'/></td>\n";
    echo "  <td class='text-center' id='td_patient_copay_$var_index' ><input type='text' class='form-control' name='patient_copay_" . attr($iname) . "'  id='patient_copay_" . text($iname) . "' " .
        " value='" . text(bucks($patcopay)) . "' onchange='coloring();calctotal()'  autocomplete='off' " .
        "onkeyup='calctotal()'/></td>\n";
    ...
    echo "  <td class='text-right'><input type='text' class='form-control' name='" . attr($iname) . "'  id='paying_" . attr($var_index) . "' " .
        ...
        "onkeyup='calctotal()'/></td>\n";
    echo "  <td class='text-right'><input class='btn btn-primary btn-sm' type='submit' id='submitbtn' value='" .htmlspecialchars(xl('+'), ENT_QUOTES) . "' />
        <div id='searchspinner'><img src='" .$GLOBALS['webroot']. "/interface/pic/ajax-loader.gif' /></td>\n";		
    echo " </form>\n";
    echo " </tr>\n";
    echo "<tr class='table-active'>
          <td class='font-weight-bold' id='td_total_1' colsan='3'>\n";
        echo "<select class='form-control' name='provd_".attr($encounter)."' id='provd_".attr($encounter)."' style='width:180px;'>";
        ...
        echo "</select>";		  
	 echo " </td>\n";
     echo "<td class='font-weight-bold' id='td_total_4'></td>
          ...
          <td class='font-weight-bold text-right'>" .xlt('Total'). "</td>
                        ...
                        <div class="col-12 oe-custom-line">
                            ...
                            <div class="pl-3">
                                <label class="radio-inline">
                                    <input checked="checked" id="radio_type_of_coverage1" name="radio_type_of_coverage" onclick="make_visible_radio();make_self();" type="radio" value="self"><?php echo xlt('Self'); ?>
                                </label>
                                <label class="radio-inline">
                            ...
                            <table class="table" id="table_display">
                                <thead>
                                    <tr class="table-active" id="tr_head">
                                        ...
                                        <td class="font-weight-bold text-center" width="50"><?php echo xlt('Accion'); ?></td>
                                    </tr>
                                </thead>
                                <?php
                                $encs = array();
                                ...
                                foreach ($encs as $key => $value) {
                                    ...
                                    if ((($NumberOfInsurance == 0 || $value['last_level_closed'] == 4 || $NumberOfInsurance == $value['last_level_closed']))) {//Patient balance
                                        ...
                                        $duept = $brow['amount'] + $srow['amount'] - $drow['payments'] - $drow['adjustments'];
											...
											$trow = sqlQuery("SELECT T.tprofesional,T.id AS tarifa_id FROM tarifas_users AS TU,tarifas AS T WHERE " .
                                            "TU.user_id = ? and T.id=TU.tarifa_id", array($pid));
											//
...
<?php
if (empty($_GET['prov']) && empty($_GET['group'])&& empty($_GET['liq'])) { ?>
    <div class="jumbotron jumbotron-fluid px-2 py-2 my-2" id="patient_details">
        <div class="form-group">
            <label for="form_patient"><?php echo xlt('Patient'); ?>:</label>
            <input class='form-control' type='text' name='form_patient' id="form_patient" style='cursor:pointer;' placeholder='<?php echo xla('Click to select'); ?>' value='<?php echo is_null($patientname) ? '' : attr($patientname); ?>' onclick='sel_patient()' title='<?php echo xla('Click to select patient'); ?>' />
...
<?php } ?>
<?php
if ($_GET['group'] === true && $have_group_global_enabled && empty($_GET['liq'])) { ?>
...
<div class="form-row mx-2">
 ...
 <!-- Done with providers now scheduling -->
<?php
...
<div class="form-row mx-2">
    <div class="col-sm form-group">
        ...
        <?php
        if ($_GET['group'] != true && empty($_GET['liq'])) {

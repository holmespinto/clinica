    ...
    if (!CsrfUtils::verifyCsrfToken($_GET["csrf_token_form"])) {
        CsrfUtils::csrfNotVerified();
							...
							$p = "SELECT * " .
                                "FROM  form_encounter" .
                                " WHERE encounter = ? ";
                                $bp = sqlStatement($p, array($_GET["key"]));
...
<html>
<body>
<form class="form-horizontal" name='theform' id='theform' method="post"  action="new_facturacion_save.php">
	<input type="hidden" name="csrf_token_form" id="csrf_token_form" value="<?php echo attr(CsrfUtils::collectCsrfToken()); ?>" />
	<input type="hidden" name="pid" value="<?php echo $_GET["pid"]; ?>" />
	<input type="hidden" name="encounter" value="<?php echo $_GET["key"]; ?>" />	
                            ...
                            <table class="table" id="table_display">
                                <thead>
								   ...
								   <tr class="table-active" id="tr_head">
										...
										<td class="font-weight-bold text-white"  style="background: gray;">Digite el valor:</td>
										<td class="font-weight-bold"  style="background: gray;">
										<input type="text" id="valor_consulta2"  name="valor_consulta2" class="form-control validanumericos" value=""  />		  										
										<script>
										...
										<td class="font-weight-bold"  style="background: gray;">
										 ...
										 <select class="form-control" id="lista_medicamento" name="lista_medicamento[]" multiple >
													<?php
												  echo "<option value='0'>Medicamentos</option>"; 
													$query3 = "SELECT * FROM prescriptions where encounter=? AND patient_id=? ORDER BY encounter";
													$bres3 = sqlStatement($query3, array($_GET['key'],$_GET['pid']));
									...
									$("#lista_medicamento option:selected").each(function () {	
										...
										var f = document.forms[0];
													obj = {};		
													$.ajax({
													url: 'json_medicamentos.php?sSearch='+$("#lista_medicamento option:selected").text().substring(0,5)+'&csrf_token_form='+ $("#csrf_token_form").val(),
													type: 'POST',
										...
										<td class="font-weight-bold"  style="background: gray;">
										<input type="text" id="valor_descuento"  name="valor_descuento" class="form-control validanumericos" value="0"  />		  										
										<script>
										...
										<td class="font-weight-bold"  style="background: gray;">
										<input type="text" id="valor_pago"  name="valor_pago" class="form-control" value=""  required />		  										
										</td>
										</tr>
										...
										<button value='btn' class='btn btn-primary btn-sm editenc btn-save'>Guardar Pago</button></a>										
										</td>
										</tr>
									</br>
                                <thead>
					...
					$("#valor_consulta").on('change', function () {
						...
						var elegido=$(this).val(); 
							...
							if (elegido==1) {
								document.getElementById("tr_consulta2").style.display = '';
								var f = document.forms[0];
								f.valor_pago.value =0;
								f.valor_consulta2.value =0;
								sessionStorage.clear();
							}else if(elegido==10){
								document.getElementById("tr_consulta2").style.display = '';
								var f = document.forms[0];

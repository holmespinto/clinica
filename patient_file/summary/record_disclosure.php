...
<body>
	...
	<div class="form-group col-lg-3">
        <label for="cups">CODIGOS CUPS</label>
        <select name="cups" id="cups" class="form-control">
            <option value="1">Capitulo 01 SISTEMA NERVIOSO</option>
            <option value="2">Capitulo 02 SISTEMA ENDOCRINO</option>
            <option value="3">Capitulo 03 SISTEMA VISUAL</option>
            <option value="4">Capitulo 03 SISTEMA VISUAL</option>
            <option value="5">Capitulo 04 SISTEMA AUDITIVO</option>
            <option value="6">Capitulo 05 NARIZ, BOCA Y FARINGE</option>
            <option value="7">Capitulo 06 SISTEMA RESPIRATORIO</option>
            <option value="8">Capitulo 07 SISTEMA CIRCULATORIO</option>
            <option value="9">Capitulo 08 SISTEMA HEMATICO Y LINFATICO</option>
            <option value="10">Capitulo 09 SISTEMA DIGESTIVO</option>

        </select>
    </div>
    <div class="form-group col-lg-3">
        <label for="subcups">SUBCOD CUPS</label>
        <select name="subcups" id="subcups" class="form-control">
            <option value="1">Sin items</option>
        </select>
    </div>
<script language="javascript">
$(document).ready(function(){
    $("#cups").on('change', function () {
        $("#cups option:selected").each(function () {
			
			//document.getElementById('#subcups').disabled = true;
            var elegido=$(this).val(); 
			 var request_method = $(this).attr("method");
			$.post("json_record_disclosure.php", { elegido: elegido,csrf_token_form:"<?php echo attr(CsrfUtils::collectCsrfToken()); ?>" }, function(data){
                $("#subcups").html(data);
            });	       
		/*
		$.ajax({
            url: "json_record_disclosure.php",
            type: request_method,
